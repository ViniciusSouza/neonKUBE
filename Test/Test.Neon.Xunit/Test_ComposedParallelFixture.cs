...
namespace TestXunit
{
    ...
    public class Test_ComposedParallelFixtures : IClassFixture<ComposedFixture>
    {
        ...
        private static ServiceMap CreateServiceMap(string serviceName)
        {
            var serviceMap = new ServiceMap();

            serviceMap.Add(new ServiceDescription() { Name = serviceName });

            return serviceMap;
        }

        public class MyService1 : KubeService
        {
            public MyService1() : base(CreateServiceMap("my-service-1"), "my-service-1")
            {
            }
        ...
        public class MyService2 : KubeService
        {
            public MyService2() : base(CreateServiceMap("my-service-2"), "my-service-2")
            {
            }
            ...
            composedFixture.Start(
                () =>
                {
                    ...
                    composedFixture.AddFixture("container", new ContainerFixture(),
                        containerFixture =>
                        {
                            containerFixture.StartAsComposed("my-container", $"nkubedev/test:latest");
                        },
                        group: 1);

                    // Start two Neon services as [group 2].

                    composedFixture.AddServiceFixture("service1", new KubeServiceFixture<MyService1>(), () => new MyService1(), group: 2);
                    composedFixture.AddServiceFixture("service2", new KubeServiceFixture<MyService2>(), () => new MyService2(), group: 2);
                });
        ...
        public async Task Verify()
        {
            ...
            var service1Fixture  = (KubeServiceFixture<MyService1>)fixture["service1"];
            var service2Fixture  = (KubeServiceFixture<MyService2>)fixture["service2"];
            ...
            NeonHelper.WaitFor(() => service1Fixture.Service.Status == KubeServiceStatus.Running, timeout: TimeSpan.FromSeconds(10));
            NeonHelper.WaitFor(() => service2Fixture.Service.Status == KubeServiceStatus.Running, timeout: TimeSpan.FromSeconds(10));
        }
    }
}
