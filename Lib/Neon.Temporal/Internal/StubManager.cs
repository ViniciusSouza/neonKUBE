...
@"        private static class ___StubHelper
        {
            ...
            private static MethodInfo       executeActivityAsync;                   // from: TemporalClient
            ...
            [MethodImpl(MethodImplOptions.NoInlining)]
            public static ActivityOptions NormalizeOptions(TemporalClient client, ActivityOptions options, System.Type interfaceType, MethodInfo method)
            {
            ...
            [MethodImpl(MethodImplOptions.NoInlining)]
            public static ChildWorkflowOptions NormalizeOptions(TemporalClient client, ChildWorkflowOptions options, System.Type interfaceType, MethodInfo method)
            {
...
");
        ...
        /// <exception cref="WorkflowExecutionException">Thrown when there are problems with the <paramref name="workflowInterface"/>.</exception>
        ...
        /// <returns>The stub instance.</returns>
        /// <exception cref="WorkflowExecutionException">Thrown when there are problems with the <typeparamref name="TWorkflowInterface"/>.</exception>
        ...
        public static DynamicActivityStub GetActivityStub(Type activityInterface)
        {
            ...
            sbSource.AppendLine();
            sbSource.AppendLine($"        public {stubClassName}(TemporalClient client, IDataConverter dataConverter, Workflow workflow, string activityTypeName, ActivityOptions options, System.Type activityInterface)");
            sbSource.AppendLine($"        {{");
            sbSource.AppendLine($"            this.client            = client;");
            sbSource.AppendLine($"            this.dataConverter     = dataConverter;");
            ...
            sbSource.AppendLine();
            sbSource.AppendLine($"        public {stubClassName}(TemporalClient client, IDataConverter dataConverter, Workflow workflow, Type activityType, LocalActivityOptions localOptions, System.Type activityInterface)");
            sbSource.AppendLine($"        {{");
            sbSource.AppendLine($"            this.client              = client;");
            sbSource.AppendLine($"            this.dataConverter       = dataConverter;");
            sbSource.AppendLine($"            this.workflow            = workflow;");
        ...
        private static string SerializeMethodParameterTypes(MethodInfo method)
        {
            ...
            else
            {
                ...
                {
                    sb.AppendWithSeparator($"typeof({TemporalHelper.TypeNameToSource(parameter.ParameterType)})", ", ");
                }
